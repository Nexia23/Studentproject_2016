...
class C_grad:
    ...
    def calc_cval(self,step, C_t):
        ...
        for i in range(len(self.C_i)):
            ...
            if self.c_ary[i].name == 'lone':  # have their default c
            ...
            for j in range(len(self.C_i)):  # nachbarzellen_c aufaddieren
                ...
                if self.c_ary[j].name == 'lone':           #produce nothing so no calculation
                    continue

                chklist =['work','maybe']

                if self.c_ary[i].name==self.c_ary[j].name:
                    fac=1

                elif all(name in [self.c_ary[i].name,self.c_ary[j].name] for name in chklist):
                    fac=self.mufac

                if j != i:

                    if self.r[j] <= self.c_ary[i].radius:          #if r[j]< radius they touch so conz should =surface
                        c_neighbor += conz_j*fac
                    else:
                        c_neighbor += conz_j*fac \
