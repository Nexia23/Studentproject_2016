        ...
        for cc in range(start,stop,step):                        #cells being placed#
            ...
            if border(cc):                                  #checks for outofbounds around edges#
                if checkneighbor(cc):                       #check if cell can be placed#
                    ...
                    if put == 0:
                        c_ary[c_num] = cl.Cell(c_num, 'work', radius, stategamble(), pos[cc][0], min(pos[cc][1],radius), maxrad)
                        occupy(cc,c_num)
                        c_num += 1

                    elif put == 1:
                        c_ary[c_num] = cl.Cell(c_num, 'lone', radius, stategamble(), pos[cc][0],min(pos[cc][1],radius), maxrad)
                        occupy(cc, c_num)
                        c_num += 1
                    elif put == 2:
                        c_ary[c_num] = cl.Cell(c_num, 'maybe', radius, stategamble(), pos[cc][0],
            ...
            if border(cc):  # checks for outofbounds around edges#
                if checkneighbor(cc):  # check if cell can be placed#
                    ...
                    if put == 0:
                        ...
                        c_ary[c_num] = cl.Cell(c_num, 'lone', radius, stategamble(), pos[cc][0],
                                               min(pos[cc][1], radius), maxrad)
                        occupy(cc, c_num)
                        c_num += 1
                    elif put == 2:
...
def divide(p):        #takes rd point and places new cell
    ...
    if c_ary[p].name=='work':
        c_ary[c_num+1] = cl.Cell(c_num+1, 'work', radius, stategamble(), xn, yn, zn)
    elif c_ary[p].name=='lone':
        c_ary[c_num+1] = cl.Cell(c_num+1, 'lone', radius, stategamble(), xn, yn, zn)
    elif c_ary[p].name == 'maybe':
        c_ary[c_num + 1] = cl.Cell(c_num + 1, 'maybe', radius, stategamble(), xn, yn, zn)
...
"""""
    ...
    if c_ary[p].radius <= 0:
        ...
        #solid walls if coordinates outside put inside
...
def picd(a):  # creats vtu data with cell situation at time step
    ...
    vtk_writer.snapshot("cell_arrangements_maybe" + str(a) + ".vtu", x_list, y_list, z_list, radii=r_list,
                        x_force=F_x,
                        y_force=F_y, z_force=F_z)
