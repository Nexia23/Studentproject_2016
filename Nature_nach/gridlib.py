...
C_on=18.0                         #signalconcentration of activ cel#
K =  18.0                         #threshold c#
...
def divide(p):        #takes rd point and places new cell
    ...
    if c_ary[p].name=='work':
        ...
        c_ary[c_num+1] = cl.Cell(c_num+1, 'lazy', radius, stategamble(), xn, yn, zn)
...
def pic(a):                     #creats vtu data with cell situation at time step
    ...
    for elem in c_ary:
        if c_ary[elem].name == 'work':
            x1_list.append(c_ary[elem].xcor)
            y1_list.append(c_ary[elem].ycor)
            z1_list.append(c_ary[elem].zcor)
            r1_list.append(c_ary[elem].radius)
            F1_x.append(fx[elem])
            F1_y.append(fy[elem])
            F1_z.append(fz[elem])

    vtk_writer.snapshot("cell_arrangements_work" + str(a) + ".vtu", x1_list, y1_list, z1_list, radii=r1_list, x_force=F1_x,
                        y_force=F1_y, z_force=F1_z)
