            ...
            if border(cc):                                  #checks for outofbounds around edges#
                if checkneighbor(cc):                       #check if cell can be placed#
                    ...
                    if put <= n:
                        ...
                        c_ary[c_num] = cl.Cell(c_num, 'toxic', radius, stategamble(), pos[cc][0],min(pos[cc][1],radius), maxrad)
                        occupy(cc, c_num)
                        c_num += 1
    else:
        setcells()
            ...
            if border(cc):  # checks for outofbounds around edges#
                if checkneighbor(cc):  # check if cell can be placed#
                    ...
                    if put <= n:
                        ...
                        c_ary[c_num] = cl.Cell(c_num, 'toxic', radius, stategamble(), pos[cc][0], pos[cc][1], pos[cc][2])
                        occupy(cc, c_num)
                        c_num += 1
...
def divide(p):        #takes rd point and places new cell
    ...
    if c_ary[p].name=='work':
        ...
        c_ary[c_num+1] = cl.Cell(c_num+1, 'toxic', radius, stategamble(), xn, yn, zn)
...
def picd(a):  # creats vtu data with cell situation at time step
    ...
    vtk_writer.snapshot("cell_arrangements_dead" + str(a) + ".vtu", x_list, y_list, z_list, radii=r_list,
                        x_force=F_x,
                        y_force=F_y, z_force=F_z)
    vtk_writer.writePVD("cell_arrangements_dead" + str(a) + ".pvd")
